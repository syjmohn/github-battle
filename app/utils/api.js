...
function getProfile(username){
    return fetch(`https://api.github.com/user/${username}${params}`)
...
function getRepos(username){
    return fetch(`https://api.github.com/user/${username}/repos${params}&per_page=100`)
    .then((res)=>res.json)
    .then((repos)=>{
        if(repos.message){
...
function calculateScore(followers,repos){
    return (followers * 3)+getStarCounts(repos)
}
